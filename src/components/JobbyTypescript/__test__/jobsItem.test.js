...
test("checking search input", () => {
  const JobsDetailPage = shallow(<JobItemDetail match={{params:{id:'dsfsdfsdfs'}}}/>);
  const JobsDetailInstance = JobsDetailPage.instance();
  const myMock1 = jest.fn(() => {
    JobsDetailInstance.jobItemDetailStore.setJobDetail(responseData);
    JobsDetailInstance.jobItemDetailStore.setApiStatus("SUCCESS");
  });
  const myMock2 = jest.fn(() => {
    JobsDetailInstance.jobItemDetailStore.setApiStatus("LOADING");
    JobsDetailInstance.jobItemDetailStore.fetchJobDetail("sdsds");
  });
  JobsDetailInstance.getJobDetail=myMock2;
  JobsDetailInstance.jobItemDetailStore.fetchJobDetail = myMock1;;
  JobsDetailInstance.getJobDetail();
  expect(myMock1).toHaveBeenCalledTimes(1);
  expect(myMock2).toHaveBeenCalledTimes(1);
});
