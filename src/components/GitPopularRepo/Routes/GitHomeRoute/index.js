...
class GitPopularRepo extends Component {
  ...
  getRepos = async () => {
    ...
    if (this.data[activeTab.id]) {
      this.setState({ repoList: this.data[activeTab.id] });
      return;
    }
    try {
  ...
  changeTab = (id) => {
    this.setState(
      { activeTab: languageList.find((item) => item.id === id) },
      this.getRepos
    );
  };
  render() {
    ...
    return (
      <div className="git-popular-repo-page">
        ...
        <div className="git-poppular-repo-content">{this.renderContent()}</div>
      </div>
    );
  }
}
